...
public class SegmentedLeastSquares {
    ...
    public void computeOptIterative() {
        ...
        //   ADD YOUR CODE HERE
        for (int i = 0; i < points.length; i++)
            opt[i] = Double.POSITIVE_INFINITY;

        for (int j = 0; j < points.length; j++)
            if (j == 0) opt[j] = 0;
            else if (j == 1 || j == 2) opt[j] = costSegment;
            else
                for (int i = 1; i <= j; i++)
                    opt[j] = Math.min(opt[i - 1] + e_ij[i][j] + costSegment, opt[j]);
    ...
    public double computeOptRecursive(int j) {
        ...
        if (j <= 2) return opt[j];
            ...
            if (opt[j] != Double.POSITIVE_INFINITY) return opt[j];
            for (int i = 1; i <= j; i++) {
                if (opt[i] == Double.POSITIVE_INFINITY)
                    computeOptRecursive(i);
                opt[j] = Math.min(opt[i-1] + e_ij[i][j] + costSegment, opt[j]);
            }
            return opt[j];
        }
